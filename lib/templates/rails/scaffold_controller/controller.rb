class <%= controller_class_name %>Controller < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @<%= orm_instance.save %>
        format.html { redirect_to(@<%= singular_table_name %>, :notice => t(:successfully_created)) }
  ...
  def update
    ...
    respond_to do |format|
      if @<%= orm_instance.update_attributes("params[:#{singular_table_name}]") %>
        format.html { redirect_to(@<%= singular_table_name %>, :notice => t(:successfully_updated)) }
        format.xml  { head :ok }
      else
  ...
  def multilingual_update
    @<%= singular_table_name %> = <%= orm_class.find(class_name, "params[:id]") %>
    @<%= singular_table_name %>.not_auto_mult=true
    respond_to do |format|
      if @<%= orm_instance.update_attributes("params[:#{singular_table_name}]") %>
  ...
  def get_data
    <%= plural_table_name %>_scope = <%= class_name %>.multilingual
