...
dhtmlXTabBar.prototype = {
    ...
    _setSizes : function(x, y) {
        ...
        this._conZone.style[this._dy] = Math.max(0, _h - _t - 2 - (this._s.skin_line_x ? 3 : 0) - this._s.tab_margin) + "px";
    ...
    _setRowSizes : function() {
        for (var i = 0; i < this._rows.length; i++) {
            ...
            this._rows[i].style[this._py] = i * (this._s.line_height - (this._s.skin_line_x ? 4 : 2)) - ((_isIE && !window.postMessage && this._s._bMode) ? 2 : 0) + "px";
    ...
    _createTab : function(id, text, size) {
        ...
        if (this._close)
            thml += "<IMG style='" + this._pxc + ":4px;" + this._py + ":4px;position:absolute;' src='" + this.imgUrl + this.skin + "/close.png' >";
        tab.innerHTML = thml;
    ...
    _img : function(tag, y, pos, a, b) {
        tag.style.backgroundImage = "url(" + this.imgUrl + this.skin + "/" + tag.parentNode.skin + "_" + this._s.mode + ".gif)";
    ...
    adjustTabSize : function(tab, size) {
        ...
        tab._size = size;
        this._img(tab.childNodes[3], 5, this._px, size - (a.left + a.right), a.left)
    },
        ...
        if (!this._content[id]) {
            ...
            d.show = function() {
                ...
                if (self._awdj || self._ahdj) {
                    ...
                    if (self._ahdj)
                        self.entBox.style.height = dim[1] + self._rows.length * (self._s.line_height - 2) + 2 + "px";
                    self._setSizes();
