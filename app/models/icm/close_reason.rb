class Icm::CloseReason < ActiveRecord::Base
  ...
  scope :list_all,lambda{
    ...
    joins("LEFT OUTER JOIN #{Irm::Company.view_name} ON #{Irm::Company.view_name}.id = #{table_name}.company_id AND #{Icm::CloseReasonsTl.table_name}.language = #{Irm::Company.view_name}.language").
