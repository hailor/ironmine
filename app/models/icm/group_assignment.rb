class Icm::GroupAssignment < ActiveRecord::Base
  ...
  scope :with_company, lambda {
    select("ct.name company_name").
        ...
        joins("LEFT OUTER JOIN #{Irm::Company.table_name} c ON ct.company_id = #{table_name}.customer_company_id")
  }
  scope :with_department, lambda {
    select("dt.name department_name").
        ...
        joins("LEFT OUTER JOIN #{Irm::Department.table_name} d ON d.id = dt.department_id = #{table_name}.customer_department_id")
  }
  ...
  scope :with_site_group, lambda {
    select("sgt.name site_group_name").
        ...
        joins("LEFT OUTER JOIN #{Irm::SiteGroup.table_name} sg ON sg.site_group_code = #{table_name}.customer_site_group_code")
  }
