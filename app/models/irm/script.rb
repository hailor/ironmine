class Irm::Script < ActiveRecord::Base
  ...
  scope :with_condition,lambda{|language|
    joins("LEFT OUTER JOIN #{Irm::Condition.view_name}  ON  #{Irm::Condition.view_name}.context_code = #{table_name}.context_code AND  #{Irm::Condition.view_name}.language = '#{language}'").    
  ...
  scope :with_action,lambda{|language|
    joins("LEFT OUTER JOIN #{Irm::Action.view_name}  ON  #{Irm::Action.view_name}.context_code = #{table_name}.context_code AND  #{Irm::Action.view_name}.language = '#{language}'").
  ...
  def self.list_all
    select_all.multilingual.with_condition(I18n.locale).with_action(I18n.locale).with_template(I18n.locale)
  end
