...
module Irm::PageHelper
  ...
  def btn_icon(icon_class)
    content_tag(:img, "", {:class => icon_class, :src => "/images/s.gif", :title => t(:remove)})  
  end
  ...
  def icon_link(icon_class = "", *args, &block)
    link_to(btn_icon(icon_class), args.first, args.second, block)
  end
end