class Irm::ActionsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.xml  { @action = Irm::Action.new
                    render :xml => @action }
    end
  end
  ...
  # GET /actions/1.xml
  def show
    @action = Irm::Action.list_all.find(params[:id])
  ...
  def get_data
    ...
    respond_to do |format|
      format.json {render :json=>to_jsonp(@actions.to_grid_json([:action_code,:name,:description,:handler,:status_meaning],count))}
    end
  end
end
