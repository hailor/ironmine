  ...
  def get_data
    ...
    if category_code.blank?
      reports_scope = Irm::Report.multilingual.query_by_group_codes(report_group_codes).
        query_by_report_purpose("REPORT").with_blank_url
    else
      reports_scope = Irm::Report.multilingual.query_by_group_codes(report_group_codes).
          query_by_report_purpose("REPORT").query_by_category_code(category_code).with_blank_url
    end
    ...
    respond_to do |format|
      format.json {render :json=>to_jsonp(@reports.to_grid_json([:name,:description, :page_controller, :page_action, :url],count))}
    end
  end
end